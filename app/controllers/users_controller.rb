...
class UsersController < ApplicationController
  ...
  def update_password
    ...
    respond_to do |format|
      if @user.update_attributes(params[:user])
        format.html { redirect_to @user, notice: 'Password Modificado Correctamente' }
        format.json { head :no_content }
      else
  ...
  def update_image
    ...
    respond_to do |format|
      if @user.update_attributes(params[:user])
        format.html { redirect_to @user, notice: 'Avatar Modificado Correctamente' }
        format.json { head :no_content }
      else
        format.html { render action: "edit_image" }
        format.json { render json: @user.errors, status: :unprocessable_entity }
      end
    end
  end
